    ...
    public partial class Form1 : Form
    {
        ...
        private void FileView(object path)
        {
            ...
            foreach (var f in di.GetFiles())
            {
                if (Flag == true)
                {
                    FileCount++; Invoke(OnFile, f.DirectoryName, f.Name, f.Length.ToString(), f.CreationTime.ToString());
                }
                else
                {
        ...
        private void FileResult(string fp, string fn, string fl, string fc)
        {
            ...
            this.lvFile.Items.Add(new ListViewItem(new string[] { fp, fn, fc, fSize })); this.tsslblResult.Text = String.Format("폴더 : {0} 개, 파일 : {1} 개", DirCount, FileCount);
        }
        ...
        private void rbtnHidden_CheckedChanged_1(object sender, EventArgs e)
        {
            ItemsClear();
            Flag = false;
            if (threadFileView != null)
